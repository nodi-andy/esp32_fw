...
#    endif
    ...
    void Web_Server::handleFileList() {
        ...
        //delete a file
            if (_webserver->arg("action") == "read" && _webserver->hasArg("filename")) {
                String filename;
                String shortname = _webserver->arg("filename");
                shortname.replace("/", "");
                filename = path + _webserver->arg("filename");
                ...
                if (!SPIFFS.exists(filename)) {
                    ...
                    if (currentfile) {
                        while (currentfile.available()) {
                            String lineData = currentfile.readStringUntil('\n');
                            content += lineData;
                        } 
                        currentfile.close();
                        JSONencoder j;
                        j.begin();
                        j.member("res", "filecontent");
                        j.member("filename", "parameter");
                        j.member("content", content);
                        jsonfile = j.end();
                        showFileList = false;
                    } else {
                        status = "Cannot read ";
