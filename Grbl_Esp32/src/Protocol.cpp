...
/*int sunrise[12] =   {900,   900,    900,    800,    800,    700,    700,    700,    1612,   800,    900,    1000};
int sunset[12] =    {1700,  1700,   1800,   1900,   2000,   2100,   2200,   2100,   1613,   1900,   1800,   1700};
...
void protocol_main_loop() {
    ...
    attachInterrupt(GPIO_NUM_27, isr_buttons, FALLING);*/
    for (;;) {
        ...
        /*if (buttonClicked) {
            char command[10] = "";
            sprintf(command, "$J=G91 X5 F200\n");
            gc_execute_line(command, 0);
            grbl_send(0, "ISR\n");
            buttonClicked = false;
        }

        if (esp_timer_get_time() > lastTime + 10000000) {
            lastTime = esp_timer_get_time();
            int curMonth = atoi(NTP.getTimeDateString (time (NULL), "%02m"));
            int curClock = atoi(NTP.getTimeDateString (time (NULL), "%02H%02M"));
            int sunrisetime = sunrise[curMonth-1];
            int sunsettime = sunset[curMonth-1];
            char msg[200];
            sprintf(msg, "TICK %d, %d, %" PRIu64 "\n" , curMonth, curClock,  lastTime);
            grbl_send(0, msg);

            protocol_execute_realtime();  // Runtime command check point.
            char command[10] = "";
            if (lastState == 0 && curClock == sunrisetime) {
                sprintf(command, "x100\n");
                gc_execute_line(command, 0);
                lastState = 1;
            }
            if (lastState == 1 && curClock == sunsettime) {
                sprintf(command, "x0\n");
                gc_execute_line(command, 0);
                lastState = 0;
            }
